...
{	
    public function checkLogin() { extract($_REQUEST);
		$result = $this->getRow("SELECT * FROM admin WHERE username='".$username."' and userpassword = '".md5($userpassword)."'");
		$users = $this->getRow("SELECT * FROM user WHERE employee_id='".substr($username,strlen(ID_PREFIX))."' and userpassword = '".md5($userpassword)."'");
		if (!empty($result)) {
			$_SESSION['admin_id'] = $result['admin_id'];
			$_SESSION['category'] = 'superadmin';
			$_SESSION['userrole'] = $result['userrole'];
			$_SESSION['username'] = $result['personname'];
			$_SESSION['lastlogin']=date("dS M, h:i A",strtotime($result['last_login']));
			$this->Query("update lead_followup_detail set lock_status=0 where lock_status=".$_SESSION['admin_id']);
		$this->redirectUrl("dashboard.php"); 
		} else if(!empty($users)) {
			$_SESSION['admin_id'] = $users['employee_id'];
			$_SESSION['category'] = 'user';
			$_SESSION['userrole'] = $users['userrole'];
			$_SESSION['username'] = $users['emp_name'];
		$this->redirectUrl("dashboard.php"); 

		} else {

		$this->redirectUrl("index.php?error=1");
	} }

	public function RecordLog($table,$page_type,$record_id,$action){
		$this->Query("insert into recordlog set table_name='".$table."', page_type='".$page_type."', record_id='".$record_id."', action_type='".$action."', userid=".$_SESSION['admin_id'].", action_date='".date("Y-m-d H:i:s")."'");
  ...
  $amount=urlencode($amount);
    ...
    $url = "https://www.msn.com/en-us/money/currencydetails/fi-".$from.$to;
    /*$get = explode("<span class=bld>",$get);
    $get = explode("</span>",$get[1]);
    $converted_currency = preg_replace("/[^0-9\.]/", null, $get[0]);
    echo number_format(round($converted_currency))." /-"; */
    ...
    curl_close($ch);
    //print_r($rawdata);
    $data = explode("truncated-string'  title='", $rawdata);
    $data = explode("'>", $data[2]);
    return number_format(trim($data[0])*$amount,3);
}
	
	public function reffcode(){
	$reffcode=chr(rand(65,91)).rand(10,99).chr(rand(65,91)).rand(0,9);
	$res=$this->getRow("select * from pu_users where reffcode='".$reffcode."'");
	if(empty($res)) return $reffcode;
	else $this->reffcode();
}
	 ...
	 public function accountSettings()
 {
	 ...
	 if($_SESSION['category']=='superadmin'){
	 	$count=$this->getResultscount("select * from admin where admin_id='".$_SESSION['admin_id']."' and userpassword='".$old_pass."'");
	 	//print_r($_REQUEST);
	 	//die("select * from admin where admin_id='".$_SESSION['admin_id']."' and userpassword='".$old_pass."'");
	 if($count>0) {
		 $this->Query("update admin set userpassword='".$user_pass."' where admin_id='".$_SESSION['admin_id']."'");
		 header("location:change_password.php?msg=suc");
	 } else header("location:change_password.php?msg=err");
	 	
	 } else  if($_SESSION['category']=='user'){
	 	$count=$this->getResultscount("select * from user where employee_id='".$_SESSION['employee_id']."' and userpassword='".$old_pass."'");
	 	//die( "select * from user where employee_id='".$_SESSION['employee_id']."' and userpassword='".$old_pass."'");
	 if($count>0) {
		 $this->Query("update user set userpassword='".$user_pass."' where employee_id='".$_SESSION['employee_id']."'");
		 header("location:change_password.php?msg=suc");
	 } else header("location:change_password.php?msg=err");
